...
def build_model():  # NN Model
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(0.05), loss=keras.losses.mean_squared_error,
...
for i in range(0, len(posX)):
    plt.plot([posX[i], posX[i] + 0.2 * math.cos(math.radians(titaEnd[i]))],
             [posY[i], posY[i] + 0.2 * math.sin(math.radians(titaEnd[i]))], 'k-')
...
# MinMaxNorm = preprocessing.minmax_scale(dataMat,(-1,1))              #Preprocessing - Normalizing data between -1
# and +1
...
history = model.fit(dataX, dataY, epochs=100)  # train the model
...
plt.clf()
plt.scatter(validate_output[:, 0], real_prediction[:, 0],
            c='b')  # Plotting Actual angles( x: desired output(Joint angles used to genarate Xe,Ye nd Titae,y: output from prediction )
...
plt.xlabel("X Axis")
plt.ylabel("Y Axis")
plt.title("Desired Trajectory Coordinates and  Predicted ")
...
for q in range(0, data_points):
    ...
    Error[q][5] = math.degrees(
        single_real_prediction[0, 2])  # Error =[Error in X cor, Errpr in Y cor, Error in Tita, Q1,Q2,Q3, q]
    Error[q][6] = q
